...
> **✅ Arquetipo 9 ACTIVADO SEGMENTADO (Intervención 1) — "El Analista de Riesgo Selectivo" (`doomer_selectivo`, 150 líderes).** Doomer SELECTIVO: neutral en el 70% de las noticias (a diferencia del Doomer perpetuo), pero ante un deterioro CLARO lee la mala noticia a plena magnitud SIN invertirla — evita que las caídas reales se diluyan. Con esto la mezcla es **1.150 líderes / 10.150 agentes / 9 arquetipos / 9 voces** (el resto de este archivo conserva la redacción canónica de 8). **Medición (IA real, muestras completas):** en ÍNDICE mejora el acierto en negativas **+7.6 pts** (38%→45.6%) con costo −2.9 en positivas (+3.1 global); en CRIPTO es **inerte** (0.0 en negativas). Por eso su peso está **SEGMENTADO POR MERCADO** (`peso_doomer_por_mercado` en `config/agentes.json`): **índice 1.0, cripto/accion/oro/petróleo 0.0** (accion pendiente de medir — es índice-like, candidata a activar). El override global `ENJAMBRE_PESO_DOOMER` (env), si se fija, manda sobre la segmentación en TODOS los mercados (medición/rollback sin desplegar; vacío = segmentado). Rollback total: poner `peso_doomer_por_mercado.indice` en 0.0 o `ENJAMBRE_PESO_DOOMER=0`. Informe: `INFORME_DOOMER_SELECTIVO_FINAL.md`.
